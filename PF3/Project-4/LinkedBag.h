...
bool LinkedBag<ItemType>::remove()
{
  ...
  if (canRemoveItem)
  {
    //Generate a random number n the rane 0 through itemCount - 1
    int number = rand() % itemCount;

  //Use a temp pointer to traverse the linked list to the node to be deleted.
    Node<ItemType>* curPtr = headPtr;
    for (int i = 0; i < number; i++)
    {
      curPtr = curPtr->getNext();
    }
    Node<ItemType>* randomNodePtr = curPtr;
